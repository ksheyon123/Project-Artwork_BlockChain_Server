...
configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./config/ArtistMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ArtistMSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for ArtistMSP..."
  exit 1
fi

# generate anchor peer2 transaction
configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./config/CompanyMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CompanyMSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for CompanyMSP..."
  exit 1
fi

# generate anchor peer3 transaction
configtxgen -profile ThreeOrgChannel -outputAnchorPeersUpdate ./config/ClientMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ClientMSP
if [ "$?" -ne 0 ]; then
  echo "Failed to generate anchor peer update for ClientMSP..."
  exit 1
fi
